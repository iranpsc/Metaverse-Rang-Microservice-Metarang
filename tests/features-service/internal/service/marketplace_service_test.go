...
func (m *MockCommercialClient) CreateTransaction(ctx context.Context, userID uint64, asset string, amount float64, action string, status int32, payableType string, payableID uint64) (*pb.Transaction, error) {
...
func setupTestMarketplaceService(t *testing.T) (*service.MarketplaceService, *MockCommercialClient, *MockNotificationClient, *MockEventBroadcaster) {
	...
	// Note: This requires actual repositories which need test database
	...
	return marketplaceService, mockCommercial, mockNotification, mockBroadcaster
}
