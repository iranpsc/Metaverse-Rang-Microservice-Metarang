...
func TestGetEvents_EventTypeDefault(t *testing.T) {
	...
	m.GetEventsFunc = func(ctx context.Context, eventType, search, date string, userID uint64, page, perPage int32) ([]*models.Calendar, int32, error) {
		gotType = eventType
		return []*models.Calendar{{ID: 1}}, 1, nil
	}
...
func TestGetEvents_WithDate(t *testing.T) {
	...
	m.GetEventsFunc = func(ctx context.Context, eventType, search, date string, userID uint64, page, perPage int32) ([]*models.Calendar, int32, error) {
		...
		return []*models.Calendar{{ID: 1}}, 5, nil
	}
	svc := service.NewCalendarService(m)
	ev, total, err := svc.GetEvents(context.Background(), "event", "", "1403/01/15", 0, 1, 10)
	if err != nil || total != 5 || len(ev) != 1 {
		t.Fatal(err, total, len(ev))
	}
}
